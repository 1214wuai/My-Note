...
## Reactor
### 单 Reactor 单进程 / 线程
![37357f3fff90e79b867edd91c860bcdd.png](en-resource://database/1102:1)
Reactor对象：监听和分发事件
Acceptor对象：获取连接
Handler对象：处理业务
...
![04c9b9a94e45a53833437f84f210d514.png](en-resource://database/1104:1)
...
### 多 Reactor 多进程 / 线程
![cba3634048c65816c6367327756d177b.png](en-resource://database/1106:1)
...
![9b94521d3b6c2928ac19a5a162237262.png](en-resource://database/1108:1)

### 非阻塞
非阻塞的read请求在数据未准备好的时候立即返回，可以继续往下执行，应用程序应该不断地轮询内核，直到数据准备好，将数据从内核缓冲区拷贝到应用程序缓冲区，read调用才可以获取到结果。
![e55448dc6b9ab19fdafab56b67cfd319.png](en-resource://database/1110:1)
注意最后一次read调用，获取数据的过程是一个同步的过程，是需要等待的过程。这里的同步是指的是内核态的数据拷贝到用户程序缓冲区的这个过程。

再来看异步I/O：
### 异步I/O
【内核数据准备好】和【数据从内核态拷贝到用户态】都不需要等待。
![9b797bb721563e3dd50f78ed1d0f2118.png](en-resource://database/1112:1)
...
Reactor时序图：
![35b6d761632e982fd4f56b95af2d518f.png](en-resource://database/1114:1)


 muduo 的整体风格受到 Netty 的影响，整个架构依照 Reactor 模式，基本与如下图所示相符：
![ca288717592634387b24417fe1638da9.png](en-resource://database/1118:1)
...
![24f354ae9c03dfcd9602156e4f1c0ec9.png](en-resource://database/1120:1)
...
![92bf6a37f8f34b94d171feff90aea65e.png](en-resource://database/1096:1)
...
![7a8a0152504a835d39607234f73f9b1a.png](en-resource://database/1098:1)
...
建立连接：
![6f3557c32097022e92d50b7d43df6548.png](en-resource://database/1122:1)
![179218587e7b210b6e06dc9cca922c43.png](en-resource://database/1126:1)
...
![9cf4d411b969cbbdc59899aea4e8b74c.png](en-resource://database/1124:1)
...
建立连接：
![1f7ce245eb5b10bb2c8a6c6c5dc08a22.png](en-resource://database/1128:1)
...
![bd133a9805d0157110a19e5060f902c6.png](en-resource://database/1134:1)
...
![a96c57b7af9885f18225c5d0a8092890.png](en-resource://database/1136:1)

## HttpServer
封装了一下TcpServer，实现的比较简单
![20f609d102ffe4d05d60bf27722b14d5.png](en-resource://database/1138:1)
